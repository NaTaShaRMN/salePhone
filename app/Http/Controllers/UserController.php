...
// return view('user/product',compact('product','relate_product'));
    return view('user/product',compact('product','relate_product','size_comment','comments'));
  }
...
// $tyPe = DB::table('brands')->where('id','=',$type)->first();
    ...
    foreach ($product_asType as $value) {
      $link = DB::table('products')
      ->select('images.link')
      ->join('images', 'products.id', '=', 'images.product_id')
      ->where('products.id',$value->id)
...
// ->join('images','products.id','=','images.product_id')
    ...
    foreach ($topSelling_product as $value) {
      $link = DB::table('products')
      ->select('images.link')
      ->join('images', 'products.id', '=', 'images.product_id')
      ->where('products.id',$value->id)
...
//DB::insert('insert into comments (product_id, comment, email, name) values (?, ?, ?, ?)', [$id_product, $comment, $email, $name]);
  ...
  public function getAccount(){
    if(!isset(Auth::user()->name)){
      ...
      return view('user.account',['orderDetail'=> $or]);
    }
  }
}
